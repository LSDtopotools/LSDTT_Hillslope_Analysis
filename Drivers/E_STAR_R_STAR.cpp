...
//
// <prefix>_Relief.bil
// <prefix>_HFR_LH.bil  - Hillslope Length
// <prefix>_Slope.bil
// <prefix>_CHT.bil - Hilltop curvature
//
...
//
// Outputs 3 csv files and a raster of hilltop patches, <prefix>_Patches_CC.bil,
...
//
// These data files can be processed using the python scripts located at https://github.com/sgrieve/ER_Star
//
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  //load dem
  LSDRaster DEM((path+prefix+"_DEM"), "bil");
  ...
  vector<int> sources = FlowInfo.Ingest_Channel_Heads((path+prefix+"_DEM_CH"), "bil");
  ...
  LSDRaster lh((path+prefix+"_HFR_LH"), "bil");

  //load Relief
  LSDRaster relief((path+prefix+"_Relief"), "bil");

  //load hilltop curvature
  LSDRaster CHT((path+prefix+"_CHT"), "bil");

  //load slope
  LSDRaster Slope((path+prefix+"_Slope"), "bil");
  ...
  //Run the connected components analysis to get hilltop patches
  LSDIndexRaster BinaryHilltops = CHT.ConvertToBinary(1, -9999);
  LSDIndexRaster HilltopPatches_unfiltered = BinaryHilltops.ConnectedComponents();
  LSDIndexRaster HilltopPatches = HilltopPatches_unfiltered.RemoveSmallPatches(MinimumPatchArea);
  ...
  HilltopPatches.write_raster((path+prefix+"_Patches_CC"),"bil");
