...
// files.
//
// Driver expects an unfilled DEM in the given directory in bil format with the name format <prefix>_DEM.bil,
// a channel heads file in the same directory with the format <prefix>_CH.bil and a floodplain raster
// <prefix>_FloodPlain.bil if floodplains are to be excluded from the analysis.
//
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  //load dem
  LSDRaster DEM((path+filename+"_DEM"), "bil");
  ...
  vector<int> sources = FlowInfo.Ingest_Channel_Heads((path+filename+"_DEM_CH"), "bil"); //swap to csv?
  ...
  if (FloodplainSwitch == 1){
    LSDIndexRaster Floodplains((path+filename+"_FloodPlain"), "bil");
  ...
  if (WriteRasters == 1){
    ...
    CHT.write_raster((path+filename+"_CHT"),"bil");
    HFR_LH.write_raster((path+filename+"_HFR_LH"),"bil");
    HFR_Slope.write_raster((path+filename+"_HFR_SLP"),"bil");
    relief.write_raster((path+filename+"_Relief"),"bil");

    //perform a hillshade
    LSDRaster Hillshade = FilledDEM.hillshade(45.0,315.0,1.0);
    Hillshade.write_raster((path+filename+"_HS"),"bil");
